...
void Con_Printf(char *fmt, ...)
{
  ...
  SendMessage(hconsole, CONM_ADDTEXT, 0, (LPARAM) buff);
  va_end(args);
  free(buff);
}
...
void Win_Printf(HWND hwnd, char *fmt, ...)
{
  ...
  va_start(args, fmt);
  vsprintf(buff, fmt, args);
  SendMessage(hwnd, CONM_ADDTEXT, 0, (LPARAM) buff);
  va_end(args);
}
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  text = (TEXT *) GetWindowLongPtr(hwnd, GWLP_USERDATA);
  switch(msg)
  {
    case WM_CREATE:
	  ShowScrollBar(hwnd, SB_VERT, TRUE);
	  text = createtext(1024);
	  SetWindowLongPtr(hwnd, GWLP_USERDATA, (LPARAM) text);
